...
. Customizing parameterized rule such as this `accounts_tmout` is very easy, as the rule does not have the timeout duration hard-coded--it is parameterized by a variable.
...
You have probably noticed strange snippets in the project's code such as `{{{ xccdf_value("var_accounts_tmout") }}}` in the `accounts_tmout` rule yaml.
...
Macros allow content authors to avoid writing complex directives such as variable substitution in rules or remediations, and they can also prevent copy-pasting of the code anywhere in the content.
